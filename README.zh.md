...
- **要求**: 
  - 当前时间必须在质押时间窗口内（`block.timestamp >= stakeStartTime && block.timestamp < stakeEndTime`）
  - 如果启用白名单模式，必须是白名单用户（通过 `whitelistCheck` 修饰符检查）
  - 质押金额 >= 最小质押金额（`minStakeAmount`）
  - 如果设置了最大总质押量，总质押量 + 当前质押金额 <= 最大总质押量
  - 奖励池余额充足（`rewardPoolBalance >= actualAccruedRewards + potentialReward`）
  - 合约未暂停（`whenNotPaused`）
  - 非紧急模式（`whenNotEmergency`）
- **事件**: 触发 `PositionCreated` 事件
...
启用/禁用白名单模式
- **参数**: `enabled` - true 启用白名单模式，false 禁用
- **要求**: 仅管理员可调用（`onlyOwner`）
...
- 紧急模式下：
  ...
  - 新质押被阻止
  - 解除质押（`unstake`）被禁用
- 说明：紧急模式下只能使用 `emergencyWithdraw` 提取本金（放弃奖励），不能使用 `unstake`
...
详细测试指南请参考：[测试指南](./docs/TESTING_GUIDE.md)
...
### 核心文档
- [脚本使用指南](./scripts/README.md) - **脚本使用完整指南（推荐）**
- [脚本架构文档](./docs/SCRIPTS_ARCHITECTURE.md) - 脚本架构详解
- [测试指南](./docs/TESTING_GUIDE.md) - 测试用例编写指南
- [产品方案详细文档](./docs/PRODUCT_PLANS.md) - **运营文档（推荐）**
