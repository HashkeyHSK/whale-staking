...
**实现**: 
- `unstake()` 函数标记位置为完整质押（`isCompletedStake = true`）
- 罚没池余额在合约中累积（`penaltyPoolBalance`）
- 分配机制由外部系统管理，不在合约内实现
...
- 外部系统可以根据 `isCompletedStake` 标记来管理罚没池分配
