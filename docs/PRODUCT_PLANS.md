...
**解锁规则**：
- **锁定期限制**：锁定期内不能解除质押，只能提取奖励
- **解锁条件**：锁定期结束后才能解除质押
- **解锁时间计算**：解锁时间 = 质押时间 + 锁定期（例如：2026-11-01 质押，锁定期 365 天，解锁时间 2026-12-31）
...
| 参数项 | 配置值 | 说明 |
|--------|--------|------|
| **最小质押门槛** | 500,000 HSK | 用户首次质押的最低金额（50万 HSK） |
| **年化收益率** | 16% | 固定年化收益率（链上自动计算） |
| **锁定期选项** | 365天 | 锁定期为365天 |
...
**示例说明**：
- 用户于 2026-11-01 00:00:00 质押，选择 365 天锁定期
- 解锁时间：2026-12-31 00:00:00（365 天后）
- 锁定期内（2026-11-01 至 2026-12-31）：只能提取奖励，不能解除质押
- 锁定期结束后（2026-12-31 之后）：可以解除质押，提取本金 + 全部奖励
