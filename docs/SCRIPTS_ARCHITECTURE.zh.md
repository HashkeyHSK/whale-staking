...
组织 `scripts/` 目录以支持 HSK Staking 单一质押池，提高代码组织性和可维护性。
...
| 脚本文件 | 状态 | 说明 |
|---------|------|------|
| `/request-early-unstake.ts` | ✅ 已完成 | 申请提前解除质押 |
| `/complete-early-unstake.ts` | ✅ 已完成 | 完成提前解除质押（等待期后） |
...
提前解除质押脚本处理提前解除质押功能，包括 7 天等待期和 50% 罚金。

#### 1. `/request-early-unstake.ts`（✅ 已完成）

为指定位置申请提前解除质押。

#### 2. `/complete-early-unstake.ts`（✅ 已完成）
