# 常见问题解答（FAQ）

本文档用于快速回复用户常见问题。

---

## 一. 奖励相关

### Q1: 奖励是线性解锁吗？是区块解锁吗？

**A:** 奖励是线性解锁，基于时间计算（不是区块解锁）。

- 奖励按时间线性累积，每天都有收益
- 与区块出块速度无关，只与时间相关
- 可以随时提取已累积的奖励

**示例**：质押 10,000 HSK，年化 8%，第 30 天可提取约 65.75 HSK 奖励。

---

### Q2: 年化收益率是什么意思？365 天年化 8% 是 365 天后得到 8% 收益吗？

**A:** 是的。年化收益率是按年计算的收益率。

- **年化 8%** 意思是：如果质押满 365 天，收益是 8%
- 实际收益 = 本金 × 年化率 × (实际质押天数 / 365天)
- 如果质押时间少于 365 天，收益按比例计算

**示例**：质押 10,000 HSK，年化 8%，质押 365 天：
- 收益 = 10,000 × 8% × (365/365) = 800 HSK

**示例**：质押 10,000 HSK，年化 8%，质押 100 天（假设提前解除质押）：
- 收益 = 10,000 × 8% × (100/365) ≈ 219.18 HSK
- 不是 800 HSK（8%）

---

### Q3: 奖励什么时候可以提取？

**A:** 锁定期内可以随时提取奖励，不需要等到锁定期结束。

- 奖励会持续累积，随时可以提取
- 提取奖励后，本金继续锁定，继续获得奖励
- 锁定期结束后，解除质押时会自动提取所有奖励

---

### Q4: 如果超过锁定期还没提取，奖励会继续增加吗？

**A:** 不会。奖励只计算到锁定期结束。

- 奖励上限 = 本金 × 年化率 × (锁定期 / 365天)
- 超过锁定期的时间不会产生额外奖励
- 建议锁定期结束后及时提取

**示例**：选择 365 天锁定期，年化 8%，实际质押了 400 天：
- 奖励仍按 365 天计算 = 10,000 × 8% = 800 HSK
- 多出的 35 天不会产生额外奖励

---

### Q5: 奖励池余额不足怎么办？

**A:** 奖励池由管理员充值维护。

- 如果奖励池余额不足，无法提取奖励
- 请联系管理员充值奖励池
- 奖励会累积，等奖励池充值后可以提取

---

## 二. 质押相关

### Q6: 质押后会获得类似 st token 的代币吗？

**A:** 不会。质押后不会产生任何代币。

- 质押后只获得一个 `positionId`（质押位置 ID）
- 没有可交易的代币
- 质押位置不可转让，绑定您的地址

---

### Q7: 最小质押金额是多少？

**A:** 
- **普通 Staking**：1 HSK 起
- **Premium Staking**：500,000 HSK 起

---

### Q8: 最大总质押量是什么意思？是单笔限制吗？

**A:** 不是单笔限制，是整个产品池的上限。

- 是所有用户质押金额总和的上限
- 单笔可以质押任意金额（只要不超过池子上限）
- 达到上限后，新用户无法质押，需要等有用户解除质押

**示例**：
- 普通 Staking 最大总质押量：1,000 万 HSK
- 如果已有 900 万 HSK 被质押，新用户最多只能质押 100 万 HSK

---

### Q9: 可以多次质押吗？

**A:** 可以。每次质押会创建一个新的质押位置。

- 每个质押位置独立计算奖励
- 可以同时持有多个质押位置
- 每个位置可以独立提取奖励和解除质押

---

### Q10: Premium Staking 为什么需要白名单？

**A:** Premium Staking 面向大户和机构，需要审核。

- 需要管理员将您的地址添加到白名单
- 只有白名单用户才能参与 Premium Staking
- 请联系管理员申请白名单

---

## 三. 锁定期和提取

### Q11: 锁定期内可以解除质押吗？

**A:** 锁定期内不能解除质押，只能提取奖励。

- **锁定期内**：只能提取奖励，本金继续锁定
- **锁定期结束后**：可以解除质押，提取本金 + 全部奖励

---

### Q12: 锁定期有多长？

**A:** 目前两个产品的锁定期都是 365 天。

- 普通 Staking：365 天
- Premium Staking：365 天

---

### Q13: 锁定期结束后必须立即提取吗？

**A:** 不是。锁定期结束后可以随时提取。

- 锁定期结束后，可以随时解除质押
- 但超过锁定期的时间不会产生额外奖励
- 建议及时提取，避免资金闲置

---

### Q14: 解除质押时，奖励会自动提取吗？

**A:** 是的。解除质押时会自动提取本金 + 全部累积奖励。

- 不需要先提取奖励再解除质押
- 一次操作即可提取本金和所有奖励

---

### Q15: 什么是紧急提取？什么时候可以用？

**A:** 紧急提取是特殊情况下提取本金的功能。

- 仅在管理员启用紧急模式时可用
- 可以随时提取，不受锁定期限制
- **只能提取本金，放弃全部奖励**
- 用于紧急情况，不建议常规使用

---

## 四. 收益计算

### Q16: 如何计算我的收益？

**A:** 收益计算公式：

```
收益 = 本金 × 年化率 × (实际质押天数 / 365天)
```

**限制**：如果实际质押天数 > 锁定期，则按锁定期计算。

**示例**：
- 本金：10,000 HSK
- 年化率：8%
- 锁定期：365 天
- 实际质押：365 天
- 收益 = 10,000 × 8% × (365/365) = 800 HSK

---

### Q17: 普通 Staking 和 Premium Staking 的收益有什么区别？

**A:** 

| 产品 | 年化收益率 | 最小质押 | 参与方式 |
|------|-----------|---------|---------|
| 普通 Staking | 8% | 1 HSK | 开放（无需审核） |
| Premium Staking | 16% | 500,000 HSK | 白名单（需审核） |

---

### Q18: 奖励是按天发放还是按月发放？

**A:** 奖励是线性累积的，按秒计算。

- 不是按天或按月发放
- 奖励持续累积，随时可以提取
- 提取时计算从上次提取到现在的所有奖励

---

## 五. 产品选择

### Q19: 我应该选择普通 Staking 还是 Premium Staking？

**A:** 

**选择普通 Staking，如果：**
- 质押金额 < 500,000 HSK
- 希望随时参与，无需审核
- 接受 8% 年化收益

**选择 Premium Staking，如果：**
- 质押金额 ≥ 500,000 HSK
- 已获得白名单资格
- 希望获得 16% 年化收益

---

### Q20: 两个产品可以同时参与吗？

**A:** 可以。两个产品完全独立。

- 可以同时在两个产品中质押
- 每个产品独立计算奖励
- 互不影响

---

## 六. 技术问题

### Q21: 如何查询我的质押信息？

**A:** 通过合约函数查询。

- 使用 `getUserPositions(address)` 查询所有质押位置
- 使用 `pendingReward(positionId)` 查询待提取奖励
- 可以通过前端界面或调用合约函数查询

---

### Q22: 质押需要支付 Gas 费吗？

**A:** 需要。所有链上操作都需要支付 Gas 费。

- 质押、提取奖励、解除质押都需要 Gas 费
- Gas 费由网络决定，不是产品收取

---

### Q23: 合约可以升级吗？会影响我的质押吗？

**A:** 合约使用可升级代理模式。

- 合约可以升级，但已存在的质押不受影响
- 质押数据会保留
- 升级由管理员执行，用户无需操作

---

## 七. 其他问题

### Q24: 大户 DID 是什么？

**A:** 大户 DID 是给质押超过 50,000 HSK 用户的特殊身份标识。

- 质押超过 50,000 HSK 的用户可以 mint 大户 DID
- 这是链下功能，与质押合约无关
- 具体规则请咨询运营团队

---

### Q25: 如果遇到问题，如何联系客服？

**A:** 请联系官方客服或管理员。

- 技术问题：联系开发团队
- 白名单申请：联系管理员
- 奖励池问题：联系管理员

---

### Q26: 质押有风险吗？

**A:** 质押存在一定风险。

- **智能合约风险**：虽然经过审计，但仍存在技术风险
- **锁定期风险**：锁定期内无法提取本金
- **奖励池风险**：如果奖励池余额不足，可能无法提取奖励
- **紧急模式风险**：紧急模式下只能提取本金，放弃奖励

建议根据自身风险承受能力谨慎参与。

---

## 八. 快速参考

### 产品对比表

| 项目 | 普通 Staking | Premium Staking |
|------|-------------|----------------|
| **年化收益率** | 8% | 16% |
| **最小质押** | 1 HSK | 500,000 HSK |
| **锁定期** | 365 天 | 365 天 |
| **参与方式** | 开放 | 白名单 |
| **最大总质押量** | 1,000 万 HSK | 2,000 万 HSK |

### 重要提醒

1. **年化收益率**：是按年计算的，不是锁定期结束后的总收益
2. **奖励上限**：奖励只计算到锁定期结束，超期不增加
3. **锁定期限制**：锁定期内不能解除质押，只能提取奖励
4. **奖励池**：奖励由管理员充值，如果余额不足可能无法提取
5. **紧急模式**：紧急模式下只能提取本金，放弃奖励

---

---

## 📚 相关文档

- [主 README](../README.md)
- [产品方案详细文档](./PRODUCT_PLANS.md) - **运营文档（推荐）**
- [产品方案执行摘要](./PRODUCT_SUMMARY.md) - 快速了解
- [双层产品方案文档](./DUAL_TIER_STAKING.md) - 技术部署文档
- [产品开发文档](./PRODUCT_PLANS_DEV.md) - 开发团队文档
- [快速开始指南](./QUICK_START_DUAL_TIER.md) - 快速部署指南
- [术语表](./GLOSSARY.md) - 术语定义
- [错误处理指南](./ERROR_HANDLING.md) - 常见错误处理

---

**文档版本**: 2.0  
**最后更新**: 2026-11  
**用途**: 用户问题快速回复参考
