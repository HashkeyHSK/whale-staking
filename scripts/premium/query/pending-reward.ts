...
async function main() {
  ...
  if (!positionId) {
    throw new Error("Please provide positionId: POSITION_ID=1 npm run query:pending-reward:premium:testnet or append argument");
  }
  ...
  if (position.isUnstaked) {
    ...
    console.log(`POSITION_ID=${positionId} npm run claim:premium:testnet`);
  }
