...
describe("Premium Staking - Unstaking Functionality", () => {
  ...
  test("应该拒绝紧急状态下的解除质押", async () => {
    ...
    await fundAccount(fixture.user1, stakeAmount + parseEther("10000000"));
    ...
    assert.strictEqual(emergencyReceipt?.status, 1, "EnableEmergencyMode transaction should succeed");
    ...
    if (!emergencyMode) {
      ...
      if (emergencyReceipt && emergencyReceipt.logs && emergencyReceipt.logs.length > 0) {
        const event = getEvent(emergencyReceipt, "EmergencyModeEnabled", fixture.staking);
        if (event && event.args) {
