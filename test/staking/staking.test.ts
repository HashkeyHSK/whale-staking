...
describe("Staking - Staking Functionality", () => {
  ...
  test("should reject staking below minimum amount", async () => {
    const stakeAmount = parseEther("500"); // Less than 1000 HSK
  ...
  test("should reject staking exceeding max total staked", async () => {
    ...
    const maxTestAmount = parseEther("10000"); // Maximum amount to use for testing
    const minStakeAmount = parseEther("1000"); // Minimum stake amount
    ...
    // Ensure we have at least testStakeAmount + minStakeAmount capacity
    if (stakeableAmount < testStakeAmount + minStakeAmount) {
      console.log("Skipping test: remaining capacity too small for testing");
      return;
    }

    // Stake up to near the limit (leave testStakeAmount + minStakeAmount)
    const amountToStake: bigint = stakeableAmount - testStakeAmount - minStakeAmount;
    ...
    const excessAmount: bigint = newRemainingCapacity + minStakeAmount;
